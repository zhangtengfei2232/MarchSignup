...
class WechatPayController  extends Controller
{
    ...
    function getPay(Request $request){
        ...
        $add_order    = WeChatPayDatabase::addStudentOrder($student_id,$phone,$out_trade_no,$pay_ways); //把订单信息存入数据
        if($add_order['code'] == 1){
            return responseToJson(1,$add_order['msg']);
        }
        $weixin_order_config = config('wechatorder');
        $key          = $weixin_order_config['key'];                   //自己设置的微信key
        $appid        = $weixin_order_config['appid'];                 //微信公众号appid
        $secret       = $weixin_order_config['secret'];                //AppSecret
        $mch_id       = $weixin_order_config['mch_id'];                //微信支付商户号
        $notify_url   = $weixin_order_config['notify_url'];            //异步回调地址，需外网可以访问
        $config       = [
           'debug'    => true,                                         // 调试设置为开启
           'app_id'   => $appid,                                       // AppID
           'secret'   => $secret,                                      // AppSecret
           'token'    => $weixin_order_config['token'],                // Token
           'aes_key'  => '',                                           // EncodingAESKey，安全模式下请一定要填写！！！
           'log'      => [                                             //日志
               'level'        => $weixin_order_config['level'],
               'permission'   => 0777,
               'file'         => $weixin_order_config['file'],
           ],
           'oauth'   => [                                              //网页授权，获取用户信息
               'scopes'       => ['snsapi_userinfo'],
               'callback'     => $weixin_order_config['callback'],
           ],
           'payment' => [
               'merchant_id'  => $mch_id,                              //微信支付商户号
               'key'          => $key,                                 //自己设置的微信key
               'cert_path'    => $weixin_order_config['cert_path'],    //证书路径设置
               'key_path'     => $weixin_order_config['key_path'],     //密匙文件
        ...
        //如果订单存入成功
            $attributes        = [
                'body'         => $weixin_order_config['body'],        //支付后的支付订单信息
                'total_fee'    => $total_fee,                          //支付金额，以'分'为单位
                'notify_url'   => $notify_url,                         //回调函数
                'out_trade_no' => $out_trade_no,                       //订单号
                'trade_type'   => $weixin_order_config['trade_type'],        //唤醒接口
            ...
            $wcPayParams    = [
                ...
                "signType"  => $weixin_order_config['signType']        //微信签名方式
            ];
            $paySign        = $this->MakeSign($wcPayParams);           //生成签名
            $wcPayParams['paySign'] = $paySign;                        //存签名
            $wcPayParams['payId']   = $add_order['datas'];             //存数据库中的订单ID
    ...
    public  function updateOrder(Request $request){
         ...
         return responseToJson(0,$reset_order['msg']);
    }
    /**生成微信签名
     * @param $sign
     * @return string
